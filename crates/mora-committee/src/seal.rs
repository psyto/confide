//! `mora-seal <dir>` — the holder's last act.
//!
...
fn main() {
    ...
    let (obligation, open_at) = mora_committee::quarter_end_obligation();
    let commitment = obligation.commitment();
    let (sealed, agents) = seal(&obligation, open_at, mora_committee::ANCHOR_SLOT, 3, 5);
