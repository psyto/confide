//! `confide-agent <share-file> <sealed.json> <now>` — one release agent, as its own process.
//!
...
use confide_embargo::{ReleaseAgent, SealedDisclosure};
use std::fs;

fn main() {
    let mut a = std::env::args().skip(1);
    let share_file = a.next().expect("usage: confide-agent <share> <sealed.json> <now>");
    let sealed_file = a.next().expect("usage: confide-agent <share> <sealed.json> <now>");
    let now: i64 = a.next().expect("usage: confide-agent <share> <sealed.json> <now>").parse().unwrap();
