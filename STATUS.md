...
1. **traction がゼロ。** **9/18 の check-in が実質の締切になった**（02 が「conversation」を
   ...
   **2026-09-16 に相手が変わった。** 発行体は**適格性の門**であって最初の買い手ではない —
   ...
   要求しており（`constraints.rs:187` `:194` `:201` `:131`、**保有者自身の口座**に対して、
   預入・借入・清算の両側で）、隙間は**4行**。そして Kamino は**既に 19本の reserve** で
   tokenized stock に貸していて、13本が実残高 ≈89,195 トークン、LTV 30〜73%。
