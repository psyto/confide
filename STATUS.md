...
465,520 口座、$23.2m、SEC の日付と 0.25% 上限が、これで全部フォームに入っている。
...
| **門を通った人はまだ誰もいない（2026-09-20）** | 設定ではなく**口座**を数える測定がどこにも無かったので `./scripts/usage-scan.sh` を書いた。保有者のいる5銘柄（Backed の Apple / NVIDIA、PreStocks の SpaceX / Anthropic）で **465,520 口座を走査し、秘匿転送に設定されたものは 0 件**。最初の走査で「400バイト超」が7件出たが、全部 `pausableAccount` と `transferHookAccount` で大きいだけだった（サイズは示唆、拡張リストが判定）。**先行者はおらず、出遅れてもいない。** web/usage.json に出力し、docs-consistency.sh に検査を追加（わざと壊して落ちることを確認済み） |
...
0h. **⚠ 提出物の背骨を入れ替えた（2026-09-22、founder の判断）。動画の再収録が残っている。**
   ...
   **founder が実物を見て、私の入れた視覚的欠陥を3つ指摘（同日）。全部直した。**
   ...
   ③ 冒頭が**30秒間、法令の文字列だけで何も動かなかった** → SEC を2場面から1場面に圧縮し、
   DvP を2場面目に。製品の動く絵が 0:23 に出る。
...
0f. **⚠ 保有分布を測った。今のピッチに不利な事実が出た（2026-09-21）** —
   ...
   **⚠ 自分のバグを1つ:** 最初の測定は全銘柄 8 decimals と決め打ちしていた。**6銘柄中4つが違う**
   ...
   - **465,520 を「市場規模」として提示してはいけない。** 発見（機能は出荷され未使用）は無傷で、
     4銘柄目・3発行体目で**確認が増えた**
...
0. **納品済みの動画が、言えない数字を1つ喋っている（2026-09-20、founder の判断）。**
   ...
   正しかった。**09-20 の再測定で 465,520 になった** — 3日で 730 動き、丸めた「千」の桁が変わった。
   0.4% のずれだが、**この提出物の売りが「全部再計算できる」ことなので、審査員が実際に
   `./scripts/usage-scan.sh` を回すのが想定読者**であり、そこだけ合わない。
   文書・ページ・提出文・YouTube 説明文は**すべて 465,520 に更新済み**。動画と台本と字幕は
