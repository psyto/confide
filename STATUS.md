...
330,266 口座、$23.2m、SEC の日付と 0.25% 上限が、これで全部フォームに入っている。
...
| **門を通った人はまだ誰もいない（2026-09-20）** | 設定ではなく**口座**を数える測定がどこにも無かったので `./scripts/usage-scan.sh` を書いた。保有者のいる5銘柄（Backed の Apple / NVIDIA、PreStocks の SpaceX / Anthropic）で **330,266 口座を走査し、秘匿転送に設定されたものは 0 件**。最初の走査で「400バイト超」が7件出たが、全部 `pausableAccount` と `transferHookAccount` で大きいだけだった（サイズは示唆、拡張リストが判定）。**先行者はおらず、出遅れてもいない。** web/usage.json に出力し、docs-consistency.sh に検査を追加（わざと壊して落ちることを確認済み） |
...
0f. **⚠ 保有分布を測った。今のピッチに不利な事実が出た（2026-09-21）** —
   ...
   **⚠ 自分のバグを1つ:** 最初の測定は全銘柄 8 decimals と決め打ちしていた。**6銘柄中4つが違う**
   ...
   - **330,266 を「市場規模」として提示してはいけない。** 発見（機能は出荷され未使用）は無傷で、
     4銘柄目・3発行体目で**確認が増えた**
...
0e. **Confidential Issuance & Redemption の評価（2026-09-21）** —
   ...
   - **生き残る主張は議決権で、そちらの方が狭くて強い。** **投票に乗数は無い** —
     持分比例の集計は持分を知る必要がある。
   - **⚠ 出典未取得（founder 提供、2026-09-21）: Backpack のトークンは議決権を伴う。**
     Backed の tracker note は伴わないのが通常。**そうであれば論証が閉じる:**

     | | Backed / xStocks | Backpack Securities |
     |---|---|---|
     | 実体 | Jersey SPV への tracker note | **1:1 償還可能な security entitlement** |
     | 議決権 | **通常なし** | **あり** |
     | SEC 条件(iii) | **要求される議決権を与えていない** | **満たしうる** |

     **SEC の株主権条件を満たせる唯一の構造が、秘匿と議決権が衝突する構造でもある。**
     持分比例の投票は record date のスナップショットで集計され、オンチェーンでは残高を読む。
     **秘匿残高は読めない → 秘匿保有者は数えられない。** registrar にその瞬間だけ証明できれば別。
     **それがこのリポジトリの primitive そのもの。registrar 応用は未構築。**
     **提出文に入れる前に Backpack の一次資料を pin すること**（`CRITERIA.md` と同じ扱い）
...
0b. ~~**CWF check-in 1 — 収録と日付の出典が残っている**~~ → **両方完了。** 収録・公開・提出済み、日程は
   ...
   **既存の収録 `video/Confide_CWF_Checkin1_20260916.mp4` は使えない。** 09-16 の週を報告していて、
   ...
   それ以上に**報告している週が違う** — スワップも 330,266 の走査も SEC も、全部この後の出来事。
...
0. **納品済みの動画が、言えない数字を1つ喋っている（2026-09-20、founder の判断）。**
   ...
   文書・ページ・提出文・YouTube 説明文は**すべて 330,266 に更新済み**。動画と台本と字幕は
