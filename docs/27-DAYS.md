...
| **Week 2** | → 09-29 | The one command works end to end for a stranger, and **prints the number**. The three-minute narrative is drafted and tested against the artifact now, not in week 4. First approaches go out with the packet attached — upside, off the critical path. **Check-in 2: opens 09-25 08:00 PDT, due 09-28 08:00 PDT** — the number the command prints, and the first thing an outside reader got wrong about it. **09-25 is also the morning Stocklana's edit window shuts, 13:00 PDT**: the final re-measure and repaste come first, five hours apart. |
| **Week 3** | → 10-06 | Coverage: the command runs over **every** live tokenized-equity mint, not one, so any row is checkable by a stranger. Any reply that did arrive is worked in; anything a reader called unanswerable is answered or recorded as unanswerable. **Check-in 3: opens 10-02, due 10-05 08:00 PDT** — what the coverage run found that the single case hid. |
| **Week 4** | → 10-12 | Submission videos cut against the week-2 narrative, evidence frozen, links and disclosures checked. **Check-in 4: opens 10-09, due 10-12 08:00 PDT** — the finished artifact, and traction stated as zero if it is zero. **It closes sixteen hours before the submission itself**, so week 4 has two deadlines on its last day and the earlier one is the check-in. |
