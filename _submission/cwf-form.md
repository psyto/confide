...
```
_submission/graphic.jpg   (1920x640, 61 KB — generated by ./scripts/graphic.sh)
```

**Founder is commissioning a designed version; this is the fallback so the required field is never
empty.** Rendered from `video/graphic.html` at 1200x630, built for the size a card actually shows:
the trade, one sentence, four zeros.

The first attempt cropped a frame of the delivered video. Honest, and unreadable — at 360px a
panel of small terminal type is texture, which is what the founder pushed back on.

**If the designed version replaces it, check the same way:** render, downscale to 360px, and look.
The exchange mark here was `U+21C4` at 44px between two 72px figures, and at card size it collapsed
into a not-equals sign — the headline read *50,000 shares ≠ $8,750,000*, the opposite of the claim.
It is two arrows drawn as CSS rules now. Nothing but looking catches that class of error.
