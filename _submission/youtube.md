...
## Description — 3035 / 5000 characters

```
All 732 tokenized stocks on Solana run on Token-2022 with confidential transfers switched ON — and every single one leaves the auditor key EMPTY. The feature is shipped, configured, and unusable. Confide is what makes that slot usable.
...
0:22 All 732 mints: privacy on, auditor key empty
0:38 The same position, seen four ways
0:53 What you actually get
1:12 A live account that reads as zero
1:23 The lender's check, verified by Solana
...
So no setting of that key is correct for a regulated issuer — and a fund holding NVDAx broadcasts its position to the whole market instead. Not a choice about publicity. The only option under which it can still answer a question.
...
• On the live mints: autoApproveNewAccounts is false, so opening a confidential account on NVDAx needs Backed's approval. The accounts here are on a mint with the same configuration.
• No claim to discharge any filing. xStocks are not Section 13(f) securities.
...
**Chapters are six, not eight.** YouTube requires every chapter to run at least ten seconds and
**silently renders none at all** if one falls short. The title scene is 9.6s and the closing scene
is 9.2s, so each is merged with its neighbour. All six are now 10.6s or longer, above the three-
chapter minimum, and the timestamps come from the measured scene boundaries rather than the script.

**The first two lines are what shows before "…more".** They carry the finding and the live link on
purpose; everything below the fold is for someone who already decided to read.
...
C (66) — leads on the number; risks reading as a data post, not a product
All 732 Tokenized Stocks on Solana Leave the Auditor Key Empty
