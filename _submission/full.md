...
transfers **ON**, auditor key **empty**. **1,869 of them**, 732 and 1,137 from two issuers with
nothing to do with each other, every mint checked rather than sampled (`./scripts/slot-scan.sh`).
...
securing the loan is public today. Both halves they need now run.
...
57 tests, 19 over the seizure program. `./scripts/healthcheck.sh` re-checks every row above against
