...
execution **by an AMM**, every fill's price, **size**, time and direction published **within ten
minutes**, and a Tier 1 name capped at **0.25% of average daily volume**.

**A desk cannot go there.** Confide settles the same trade off the tape: tokenized stock against
cash, **one Solana transaction**, two signatures, and neither side publishes what moved. Delivery
versus payment is what a clearing house is for — neither party goes first, so finance inserts a
...
Every tokenized stock on Solana already ships the feature this needs, and **all 1,992** run
Token-2022 with confidential transfers **on** and the auditor slot **empty** — three unrelated
issuers, every mint checked rather than sampled.
...
built, not about them. **PYUSD and USDG land on the identical configuration** — PayPal's dollar ships the
same unusable privacy feature behind the same door.

## Working end-to-end, devnet, today

| | |
|---|---|
| **Two strangers, four files** | `swap-offer` → `swap-accept` → `swap-settle` → `swap-sign`, two machines sharing nothing but public keys — no escrow, no custodian, no oracle. `4t6HxA36…`, 2 signatures |
...
- **The lending half is parked.** Kamino runs 19 live markets in these tokens — **$23.2m deposited,
  ...
  ciphertext and settles a default without the borrower, on devnet. But a lender who cannot read a
