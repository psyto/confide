...
**ON**, auditor key **empty**. **1,992 of them**, three issuers with nothing to do with each other — two
tokenize listed equity, one tokenizes companies with no public market at all. Every mint checked,
not sampled (`./scripts/slot-scan.sh`).
...
their own prices, **$22.0m deposited**, **$83.0m their caps authorise**. SPYx 73% LTV, NVDAx 55%,
SpaceX 40%. Every position is public, and that is the only way in — Kamino's program
...
| The auditor slot, filled | one `UpdateMint` on a mirror of NVDAx's confidential-transfer setup |
| A disclosure bound to a date | 147 bytes over **that account's own ciphertext**; a figure restated later will not open it |
| Opening on schedule without the holder | five processes; the holder's exited in September, what the rest publish opens it |
...
- **On the live mints.** The issuer signs per escrow, as above. On the mirror that approval is a
  step you can watch; on `NVDAx` it is a conversation nobody has had.
